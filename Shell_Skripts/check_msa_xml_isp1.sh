...
#
# Ausgabe im Icinga:
# OK | Active-Active ULP | Redundant wird im Icinga2 als OK angezeigt
#
# Voraussetzungen:
...
# /api/show/disks 
# /api/show/fans
# /api/show/network-parameters
# /api/show/pools
# /api/show/power-supplies
# /api/show/provisioning 
# /api/show/system
# /api/show/volume-groups
# /api/show/frus
# /api/show/redundancy-mode
#
...
z_invalid_data=0
z_power_off=0
z_active_active_ulp=0
z_single_controller=0
z_failed_over=0
z_down=0
z_redundant=0
z_operational_but_not_redundant=0
...
for ((i=0; i<${#health[@]}; i++)); do
        ...
        elif [[ "${health[$i]}" = "Active-Active ULP" ]] ; then
                echo -e "\nDer Status der Komponente ist ${health[$i]}"
                z_active_active_ulp=$((z_active_active_ulp+1))
        elif [[ "${health[$i]}" = "Single Controller" ]] ; then
                echo -e "\nDer Status der Komponente ist ${health[$i]}"
                z_single_controller=$((z_single_controller+1))
        elif [[ "${health[$i]}" = "Failed Over" ]] ; then
                echo -e "\nDer Status der Komponente ist ${health[$i]}"
                z_failed_over=$((z_failed_over+1))
        elif [[ "${health[$i]}" = "Down" ]] ; then
                echo -e "\nDer Health Status der Komponente mit Name:${name[$i]} ID:${id[$i]} ist ${health[$i]}"
                z_down=$((z_down+1))
        elif [[ "${health[$i]}" = "Redundant" ]] ; then
                echo -e "\nDer Status der Komponente ist ${health[$i]}"
                z_redundant=$((z_redundant+1))
        elif [[ "${health[$i]}" = "Operational but not redundant" ]] ; then
                echo -e "\nDer Status der Komponente ist ${health[$i]}"
                z_operational_but_not_redundant=$((z_operational_but_not_redundant+1))
...
if [[ $z_gesamt -eq $z_ok ]] || [[ $z_gesamt -eq $z_active_active_ulp ]] || [[ $z_gesamt -eq $z_redundant ]] ; then
          exit $STATE_OK
     elif [[ $z_fault -gt 0 ]] || [[ $z_power_off -gt 0 ]] || [[ $z_failed_over -gt 0 ]] || [[ $z_down -gt 0 ]] ; then
          exit $STATE_CRITICAL
     elif [[ $z_degraded -gt 0 ]] || [[ $z_unknown -gt 0 ]] || [[ $z_n_a -gt 0 ]] || [[ $z_absent -gt 0 ]] || [[ $invalid_data -gt 0 ]] || [[ $z_single_controller -gt 0 ]] || [[ $z_operational_but_not_redundant -gt 0 ]] ; then
          exit $STATE_WARNING
     else
