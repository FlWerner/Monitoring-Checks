...
#
# Ausgabe im Icinga:
# OK wird im Icinga2 als OK angezeigt
#
# Voraussetzungen:
...
health=($(curl -ks -H "sessionKey: $sessionkey" https://$hostname/api/show/$show | xmlstarlet sel -t -v '//RESPONSE/OBJECT/PROPERTY[@name="'"$cli"'"]'))
...
for ((i=0; i<${#health[@]}; i++)); do
                z_gesamt=$((z_gesamt+1))
        if [[ "${health[$i]}" = "OK" ]] ; then
                echo -e "\nDer Health Status der Komponente mit ID:${id[$i]} ist ${health[$i]}"
                z_ok=$((z_ok+1))
        elif [[ "${health[$i]}" = "Warning" ]] ; then
                echo -e "\nDer Health Status der Komponente mit ID:${id[$i]} ist ${health[$i]}"
                z_warning=$((z_warning+1))
        elif [[ "${health[$i]}" = "Error" ]] ; then
                echo -e "\nDer Health Status der Komponente mit ID:${id[$i]} ist ${health[$i]}"
                z_error=$((z_error+1))
        elif [[ "${health[$i]}" = "Unavailable" ]] ; then
                echo -e "\nDer Health Status der Komponente mit ID:${id[$i]} ist ${health[$i]}"
                z_unavailable=$((z_unavailable+1))
        elif [[ "${health[$i]}" = "Unrecoverable" ]] ; then
                echo -e "\nDer Health Status der Komponente mit ID:${id[$i]} ist ${health[$i]}"
                z_unrecoverable=$((z_unrecoverable+1))
        elif [[ "${health[$i]}" = "Unknown" ]] ; then
                echo -e "\nDer Health Status der Komponente mit ID:${id[$i]} ist ${health[$i]}"
                z_unknown=$((z_unknown+1))
        elif [[ "${health[$i]}" = "Not Installed" ]] ; then
                echo -e "\nDer Health Status der Komponente mit ID:${id[$i]} ist ${health[$i]}"
                z_not_installed=$((z_not_installed+1))
        elif [[ "${health[$i]}" = "Unsupported" ]] ; then
                echo -e "\nDer Health Status der Komponente mit ID:${id[$i]} ist ${health[$i]}"
                z_unsupported=$((z_unsupported+1))
...
if [[ $z_gesamt -eq $z_ok ]] ; then
          exit $STATE_OK
     elif [[ $z_error -gt 0 ]] || [[ $z_unavailable -gt 0 ]] || [[ $z_unrecoverable -gt 0 ]] || [[ $z_unknown -gt 0 ]] || [[ $z_not_installed -gt 0 ]] ; then
          exit $STATE_CRITICAL
     elif [[ $z_warning -gt 0 ]] || [[ $z_unsupported -gt 0 ]] ; then
          exit $STATE_WARNING
     else
